...
try:
    ...
    request_file = open('../config.json')
    client_config_data = json.load(request_file)
    # Extract necessary information
    image_name = client_config_data["config"]["image"]
    container_name = client_config_data["config"]["container_name"]
    binding_ports = [f"{v}:{k}" for k, v in client_config_data["config"]["binding_port"].items()]
